...
def observed_fusion_attention(module: nn.Module, query: torch.Tensor, key: torch.Tensor,
    ...
    The private context helper is shared deliberately: duplicating packed-mask
    normalization here could silently change the attention kernel contract.
    ...
    """
    context = _prepare_fusion_attention_context(module, query, key, value, attention_mask, kwargs)
    result = torch_npu.npu_fusion_attention(
        context.query, context.key, context.value, context.query.shape[1], context.input_layout,
        pse=None, padding_mask=None, atten_mask=context.attention_mask,
        scale=context.head_dim**-0.5 if scaling is None else scaling,
        pre_tockens=context.pre_tokens, next_tockens=context.next_tokens,
        keep_prob=1.0 - dropout, inner_precise=0, sparse_mode=context.sparse_mode,
        actual_seq_qlen=context.query_lengths, actual_seq_kvlen=context.key_lengths)
    with torch.no_grad():
    ...
    if context.is_packed:
        return result[0].reshape(context.batch_size, context.query_length, *result[0].shape[1:]), None
...
class JTDeepseekV3MLAAttention(MLAAttention):
    ...
    def _project_latents(self, hidden_states: torch.Tensor) -> _MLALatents:
        """Gather compressed SP latents after separate down projections and norms.

        The Hyper latent descriptor keeps the shared key/value expansion path.
        Splitting the fused master parameter also preserves the two input-gradient
        GEMMs required by the reference graph; parameter ownership is unchanged.
        """
        ...
        return _MLALatents(batch, sequence, query_pass, query_rope.transpose(1, 2),
                           kv_latent.reshape(batch, 1, sequence, self.kv_lora_rank),
                           key_rope.reshape(batch, 1, sequence, self.qk_rope_head_dim))
...
class JTDeepseekV3MoE(DeepseekV32MoE):
    ...
    def __init__(self, config: Any, *, is_mtp: bool = False) -> None:
        ...
        nn.Module.__init__(self)
        self.config = config
        self.reference_config = config.jt_config
        self.reference_is_mtp = is_mtp
        self.padding = config.n_routed_experts if self.reference_config["use_pad_tokens"] else 0
...
class JTDeepseekV3ForCausalLM(DeepseekV32ForCausalLM):
    ...
    def __init__(self, config: Any) -> None:
        ...
        """
        DeepseekV32PreTrainedModel.__init__(self, config)
        if not hasattr(config, "jt_config"):
            raise ValueError("JTDeepseekV3ForCausalLM requires an explicit jt_config")
